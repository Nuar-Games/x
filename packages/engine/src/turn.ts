...
function runTurnStartDraw(state: GameState, events: EngineEvent[]): GameState {
  ...
  if (drawnId === undefined) {
    // GAME_RULES.md §19: an empty deck at the normal draw ends the match and Zone X is scored.
    // Scoring is X1 implementation step 14 (ARCHITECTURE.md §18).
...
/** Returns the new state, or null if the player must discard first. */
 ...
 * Stops at: HAND_LIMIT_ENFORCEMENT (over limit), REQUIRED_VS_DEPLOYMENT,
 * START_OF_TURN_VS_ACTION, and every later stage not automated yet.
 */
...
function applyDiscardForHandLimit(state: GameState, command: DiscardForHandLimitCommand): CommandResult {
  ...
  next = withInstances(
    next,
    command.cardInstanceIds.map((id) => ({ ...state.cardInstances[id]!, zone: "ZONE_TEPI" as const }))
  );
...
function applyDeployVs(state: GameState, command: DeployVsCommand): CommandResult {
  ...
  const nextPlayer: PlayerState = {
    ...player,
    hand: player.hand.filter((id) => id !== command.cardInstanceId),
    vs: command.cardInstanceId,
    vsPosition: command.position
  };
  let next = withPlayer(state, nextPlayer);
...
function applyReplaceVs(state: GameState, command: ReplaceVsCommand): CommandResult {
  ...
  const events: EngineEvent[] = [
    ...
    },
    {
      ...
      playerId: command.playerId,
      oldInstanceId: oldVs,
      newInstanceId: command.cardInstanceId,
  ...
  next = withInstances(next, [
    { ...state.cardInstances[oldVs]!, zone: "ZONE_X" },
    { ...state.cardInstances[command.cardInstanceId]!, zone: "VS" }
  ]);
  next = moveToEffectActions(next, events);
  return { accepted: true, state: next, events };
}

/** Validates and applies one player command (ARCHITECTURE.md §7). Never partially applies an illegal command. */
export function applyCommand(state: GameState, command: Command): CommandResult {
  ...
  switch (command.type) {
    case "DISCARD_FOR_HAND_LIMIT":
      return applyDiscardForHandLimit(state, command);
    case "DEPLOY_VS":
      return applyDeployVs(state, command);
    case "KEEP_VS":
      return applyKeepVs(state, command);
    case "CHANGE_VS_POSITION":
      return applyChangeVsPosition(state, command);
    case "REPLACE_VS":
      return applyReplaceVs(state, command);
  }
}
